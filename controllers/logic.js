...
moneyTransfer = (req, res) => {
    const { sAcno, rAcno, amount, psw, date, time } = req.body
    //convert to number
            ...
            users.findOne({ acno: rAcno }).then(recevier => {
                if (recevier) {
                    if (amnt <= sender.balance) {
                        ...
                        sender.transactions.push({ tAcno: rAcno, amount: amnt, type: "DEBIT", date, time })
                        sender.save()

                        //recevier data updated
                        recevier.balance = recevier.balance + amnt
                        recevier.transactions.push({ tAcno: sAcno, amount: amnt, type: "CREDIT", date, time })
                        recevier.save()
